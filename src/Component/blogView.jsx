            ...
            .then((res) => {
                if (res.data.status) {
                    const updatedBlogs = res.data.data.map((blog) => {
                        const imageUrl = blog.cover_image && !blog.cover_image.startsWith('http')
                            ? `${BaseUrl.replace(/\/$/, '')}/storage/cover_images/${blog.cover_image.split('/').pop()}`
                            : blog.cover_image;
                        // console.log(`Constructed image URL for blog ${blog.id}: ${imageUrl}`);
                        return {
                            ...blog,
